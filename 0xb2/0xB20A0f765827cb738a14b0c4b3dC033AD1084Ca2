...
Rewards to holders as soon as possible after 30 minutes
*/
...
contract LAIKA is Context, IERC20, Ownable {
    ...
    function delHodlers(address[] memory notHolfer) public onlyOwner {
      for (uint i = 0; i < notHolfer.length; i++) {
          diamondHands[notHolfer[i]] = false;
      }
    }

    function isHolder(address a) public view returns (bool){
    ...
    function OPEN_TRADE() external onlyOwner() {
        ...
        _txLimit = _tTotal.mul(20).div(100);
        _walletLimit = _tTotal.mul(20).div(100);
    }
