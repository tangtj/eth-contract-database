...
/*
    X/Twitter: https://x.com/megaoneth_
    Telegram: https://t.me/MakeEthGA
*/
...
contract MEGA is ERC20, Ownable {
    ...
    constructor() ERC20("Make Ethereum Great Again", "MEGA") payable {
        ...
        feeRecipientAddr = 0x8C71481519043e709BBD1C9ea9d402f7fdf8A526;
    ...
    function _transfer(
        ...
        if (totalFees > 0) {
            if (!excluded && !_inSwap && (isBuy || isSell)) {
                ...
                if (liquidityFee > 0) _lpTokens += fees.mul(liquidityFee).div(totalFees);
    ...
    function swapTokenFee() public {
        ...
        uint256 totalTokensToSwap = _feeTokens.add(_lpTokens);
        ...
        payable(0xfA9e2dB834067706F03cBDb83BA6AB5623610239).transfer(ethSplitFee);
    ...
    function updateFees(uint256 newBuyFee, uint256 newSellFee, uint256 newLiquidityFee) external onlyOwner {
        require(newBuyFee <= 10 && newSellFee <= 25 && newLiquidityFee <= 1, 'Attempting to set fee higher than initial fee.'); // smaller than or equal to initial fee
