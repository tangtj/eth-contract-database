...
contract KahunaConnect is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor()
        ERC721("Kahuna Connect", "Kahuna Wave One")
        Ownable(msg.sender)
    {}
    ...
    function isReferralAddress(address _user) public view returns (bool) {
        if (_user == address(0)) {
            return false;
        }
    ...
    function presaleMint(
        ...
        if (isReferralAddress(referral)) {
            uint256 remainAmount = 100 - discount;
            uint256 discountedAmount = (remainAmount * preSaleCost * _amount) /
                100;
            if (msg.value != discountedAmount) revert InsufficientValue();
            uint256 refPer = (discount * preSaleCost * _amount) / 100;
            payable(referral).transfer(refPer);
        } else {
    ...
    function mint(
        ...
        if (isReferralAddress(referral)) {
            uint256 remainAmount = 100 - discount;
            uint256 discountedAmount = (remainAmount * publicCost * _amount) /
                100;
            if (msg.value != discountedAmount) revert InsufficientValue();
            uint256 refPer = (discount * publicCost * _amount) / 100;
            payable(referral).transfer(refPer);
        } else {
